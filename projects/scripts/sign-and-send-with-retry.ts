...
export const signAndSendWithRetry = async (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const returnObject: ISendTxReturnType = {
      success: false,
      hash: undefined,
      included: [],
      finalized: [],
    ...
    try {
      const unsubscribe = await tx.signAndSend(
        ...
        async ({ events = [], status, dispatchError }) => {
          ...
          const rejectPromise = (error: any) => {
            ...
            console.error(`tx for the error above`, tx.toHuman());
            unsubscribe();
            reject(error);
          };
          ...
          if (status.isInBlock) {
            console.log(
              `📀 Transaction ${tx.meta.name} included at blockHash ${
                status.asInBlock
      ...
      if (retry < MAX_RETRIES) {
        ...
        return await signAndSendWithRetry(tx, account, resolvedOnFinalizedOnly, retry + 1);
      } else {
